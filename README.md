...
* **RF04:** O sistema deve permitir o registro de **Entrada de Estoque**, atualizando a quantidade do produto e registrando a movimentação.
...
* `cnpj`
* `email`
* `@OneToMany(mappedBy="fornecedor") List<Produto>` (Um fornecedor para muitos produtos)
...
* `cpf`
* `email`
* `@OneToMany(mappedBy="cliente") List<Venda>` (Um cliente para muitas vendas)
...
* `@OneToMany(mappedBy="venda", cascade=CascadeType.ALL) List<ItemVenda>` (Uma venda para muitos itens)
...
* `@ManyToOne Produto`
* `quantidade`
* `precoUnitario` (Preço no momento da venda)
...
* **Árvore Binária**: Uma árvore binária será usada na camada de visão para exibir a lista de produtos sempre ordenada alfabeticamente de forma eficiente.
